        ...
        case MENU:
            ...
            if (IsCursorHidden()) EnableCursor();
                ...
                if (CheckCollisionPointRec(mousePos, instructdest) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                    currentScreen = INSTRUCT; // Tell the game to switch to instructions state
                }
        ...
        switch (currentScreen)
        {
            ...
            case GAMEOVER:
            {
                ...
                if (isHovered && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                    ...
                    enemy.active = true;         // Reactivate the monster

                    // load the 1st level again to reset positions and state
                    LoadLevel(1, &player, &enemy, &mazeKey, &state);
